...
const (
	ConfFileEnvVar  = "RKN_CONF" // env variable stores a path to the config file
	DefaultConfFile = "rkn.toml" // default path to the config file
)
...
type (
	...
	Cfg struct {
		...
		Res struct {
			...
			Attempts     int           `toml:"attempts"`
			GetTimeout   time.Duration `toml:"download_timeout"`
			RetryTimeout time.Duration `toml:"retry_timeout"`
		} `toml:"res"`
		Parse struct {
			FromDump            *string       `toml:"from_dump"`
			ProgressPollTimeout time.Duration `toml:"progress_poll_timeout"`
			BogusIp             struct {
			...
			Redis struct {
				...
				TimeoutConn time.Duration `toml:"timeout_conn,omitempty"`
				TimeoutRead time.Duration `toml:"timeout_read,omitempty"`
			} `toml:"redis"`
		} `toml:"parse"`
		Lists struct {
